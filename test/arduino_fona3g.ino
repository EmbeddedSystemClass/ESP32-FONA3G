...
void setup() {
  ...
  Serial.begin(115200);
  Serial2.begin(115200, SERIAL_8N1, FONA_RX, FONA_TX);
...
void printMenu(void) {
  ...
  Serial.println(F("[~] Go To AT Command Mode"));
  Serial.println(F("-------------------------------------"));
  Serial.println(F(""));
...
void loop() {
  ...
  if(mode)
  {
    while(Serial.available())
    {
      ...
      {
        mode = 0;
        Serial.println("Going back to menu mode");
      }else{
        Serial2.write(a);
      }
    }
  ...
  switch (command) {
    case '~':{
      Serial.println("Going Into AT Command Mode");
      mode = 1;
      break;
    }
    case '?': {
